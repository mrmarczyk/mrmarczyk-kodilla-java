...
public class BoardTestSuite {
    ...
    @Test
    public void testAddTaskListAverageWorkingOnTask1stSolution() {
        //Given
        Board project = prepareTestData();
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
    ...
    @Test
    public void testAddTaskListAverageWorkingOnTask2ndSolution() {
        //Given
        Board project = prepareTestData();
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        ...
        long taskCount = project.getTaskLists().stream()
                ...
                .flatMap(tl -> tl.getTasks().stream())
                .count();
        long workingDaysSum = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .mapToLong(t -> DAYS.between(t.getCreated(), LocalDate.now()))
                .sum();
